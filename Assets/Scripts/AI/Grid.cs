...
public GameObject nodeModel;
    ...
    void simple(Vector2Int startNode, Vector2Int finishNode)
    {
    ...
    void dijkstra(Vector2Int startNode, Vector2Int finishNode)
    {
        ...
        //  Очередь вершин в обработке - в A* необходимо заменить на очередь с приоритетом
        PriorityQueue<Vector2Int> nodes = new PriorityQueue<Vector2Int>();
        //  Начальную вершину помещаем в очередь
        ...
        while (nodes.Count != 0)
        {
            ...
            foreach (var node in neighbours)
            {
                var newDistance = grid[current.x, current.y].Distance + PathNode.Dist(grid[node.x, node.y], grid[current.x, current.y]);
                if (grid[node.x, node.y].walkable
                     && (!visited.Contains(node) || grid[node.x, node.y].Distance > newDistance))
                {
                    grid[node.x, node.y].Distance = newDistance;
                    // различие двух алгоритмов в дополнительном слагаемом на данном шаге 
                    var priority = newDistance;// + Heuristic(node, finishNode);
                    grid[node.x, node.y].ParentNode = grid[current.x, current.y];
    ...
    void getPath(Vector2Int finishNode)
    {
        ...
        while (pathElem != null)
        {
            totalDistance += pathElem.Distance;
            pathElem.Illuminate();
            pathElem = pathElem.ParentNode;
        }
        UnityEngine.Debug.Log(totalDistance);
    }
    ...
    void Update()
    {
        ...
        System.Console.WriteLine(grid.GetLength(0));
